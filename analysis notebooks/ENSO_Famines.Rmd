...
famine_periods <- data %>%
  group_by(Region) %>%
  mutate(
    famine_id = cumsum(Famine_start == 1)   # cumulative count of famine starts
  ) %>%
  filter(Famine > 0) %>%                   # keep only famine periods
  ungroup()
...
```{r}
#correlation between nino34 and PDSI
cor(data$nino34, data$PDSI)
```
...
m_lin <- feols(
  ...
  data = data %>% filter(Region == "Central Europe"),
  vcov = DK ~ Year,
