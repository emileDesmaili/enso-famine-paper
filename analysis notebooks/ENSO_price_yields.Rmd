...
```{r}
data <- read.csv('../data/price_2023_enso.csv')
...
data_region <- read.csv('../data/famine_region_data.csv')
...
```{r}
est1 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = data, panel.id = c("Location", "Year"), vcov = DK ~ Year)
est2 <- feols(logprice ~ Year:Location + l(nino34,0:3)  + i(Decade) + PDSI + temp_summer + temp_winter +precip_summer + precip_winter | Location , data = data, panel.id = c("Location", "Year"), vcov = DK ~ Year)
est3 <- feols(logprice ~ Year:Location + l(nino34,0:3)  + i(Decade) + ongoing_wars + log(1+Deaths) | Location, data = data, panel.id = c("Location", "Year"), vcov = DK ~ Year)
est4 <- feols(logprice ~ Year:Location + l(nino34,0:3)  + i(Decade) + PDSI + temp_summer + temp_winter +precip_summer + precip_winter + ongoing_wars + log(1+Deaths) | Location, data = data, panel.id = c("Location", "Year"), vcov = DK ~ Year)
...
etable(est1, est2, est3,est4,
      ...
      keep = "nino",
       ...
       dict = dict,
       drop.section = "fixef",
      title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Grain Prices",
      extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                   "Location Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                  "Controls" = c("No", "Climate", "Conflict", "Climate+Conflict")),
...
```{r}
data_federico <- read.csv('../data/federico_prices_enso.csv') %>% 
  mutate(Decade = floor(Year / 10) * 10) %>% 
  dplyr::select(Location,Year,Decade, everything()) %>% 
  drop_na() %>% 
  distinct() %>% 
...
control_dict <- list(
  "Base" = "Decade",
  "Climate" = c("Decade", "PDSI", "temp_summer", "temp_winter", "precip_summer", "precip_winter"),
  "Conflict" = c("Decade", "ongoing_wars", "Deaths"),
  "Climate + Conflict" = c("Decade", "PDSI", "temp_summer", "temp_winter", "precip_summer", "precip_winter", "ongoing_wars", "Deaths"
))
...
```{r}
wheat_dta <- read_dta('../data/wheatprices_dataset.dta') %>%
...
control_dict <- list(
  "Base" = "decade",
  "Climate" = c("decade", "PDSI_europe", "growseasontemp", "meantemp", "growseasonrain"),
  "Conflict" = c("decade", "ongoing_wars", "Deaths"),
...
#save
ggsave("../figures/plots/figA_irf_wheatprice.pdf", device = cairo_pdf, dpi=300)
```
...
```{r}
est1 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = data, panel.id = c("Location", "Year"), vcov = DK ~ Year)
est2 <- feols(logprice ~ Year:Location + l(nino34,0:3)  + i(Decade)| Location , data = data, panel.id = c("Location", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=200))
est3 <- feols(logprice ~ Year:Location + l(nino34,0:3)  + i(Decade)| Location , data = data, panel.id = c("Location", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=500))

est4 <- feols(logprice ~ Year:Location + l(nino34,0:3)  + i(Decade)| Location , data = data, panel.id = c("Location", "Year"), vcov = cluster~ Location+Year)
...
etable(est1, est2, est3,est4,
       ...
       label = "tab:grainprice_SEcheck",
          title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Grain Prices under different Standard Errors",
       drop.section = "fixef",
       extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes","Yes"),
                   "Location Fixed Effects" = c("Yes", "Yes", "Yes","Yes"),
...
yield_dta <- read_dta('../data/yieldratio_dataset.dta') %>%
  left_join(data_region %>% 
              dplyr::select(Year, nino34,nino12,nino3,nino4, PDSI_europe, NAO_cal, JSL,Deaths,ongoing_wars,Death_ratio,precip_summer_europe), 
            by = c("year" = "Year")) %>%
  mutate(decade = floor(year / 10) * 10) %>% 
...
```{r}
yield_2023 <- read.csv('../data/yield_ljungqvist_2025.csv') %>% 
  mutate(decade = floor(Year / 10) * 10) %>% 
...
```{r}
est1 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10) + i(decade) | VarLocationGrain, data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = ~VarLocationGrain + Year)

est2 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10) + i(decade) + PDSI  + temp_summer + temp_winter + precip_summer+ precip_winter| VarLocationGrain, data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"))


est3 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10) + i(decade) + ongoing_wars + log(1+Deaths) | VarLocationGrain, data = yield_2023 %>%  filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"))

est4 <- feols(logyield ~ Year:LocationGrain + l(nino34,0:3):i(teleco_PDSI_10) + i(decade) + ongoing_wars + log(1+Deaths) + temp_summer_europe + temp_winter_europe + precip_summer_europe + precip_winter_europe | VarLocationGrain, data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain","Year"))
...
etable(est1, est2, est3,est4,
       ...
       dict = dict,
        ...
        title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Wheat and Rye Grain Harvests (Yields and Tithes).",
       drop.section = "fixef",
       extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes","Yes"),
                   "Record-Location-Grain Fixed Effects" = c("Yes", "Yes", "Yes","Yes"),
...
```{r}
est1 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10) + i(decade) | VarLocationGrain, data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = ~VarLocationGrain + Year)

est2 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10) + i(decade) + PDSI  + temp_summer + temp_winter + precip_summer+ precip_winter| VarLocationGrain, data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"))


est3 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10) + i(decade) + ongoing_wars + log(1+Deaths) | VarLocationGrain, data = yield_2023 %>%  filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"))

est4 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10) + i(decade) + ongoing_wars + log(1+Deaths) + temp_summer_europe + temp_winter_europe + precip_summer_europe + precip_winter_europe | VarLocationGrain, data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain","Year"))
...
etable(est1, est2, est3,est4,
       ...
       dict = dict,
        ...
        title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Grain Harvests (Rye and Wheat).",
       drop.section = "fixef",
       extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes","Yes"),
                   "Record-Location-Grain Fixed Effects" = c("Yes", "Yes", "Yes","Yes"),
                   'Controls' = c("None", "Climate", "Conflict", "Climate + Conflict")),
...
```{r}
est1 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10) + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")) , panel.id = c("VarLocationGrain", "Year"), vcov = DK ~ Year)
est2 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10)  + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=200))
est3 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10)  + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=500))
est4 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_PDSI_10)  + i(decade)| VarLocationGrain , data = yield_2023 %>% filter( Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"), vcov = cluster~ loc_id+Year)
...
etable(est1, est2, est3,est4,
       ...
       drop.section = "fixef",
      title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Grain Harvests (Rye and Barley) under different Standard Errors",
       extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                   "Record-Location-Grain Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                   "Standard Errors" = c("Driscoll-Kray (L=4)", "Conley (200 km)","Conley (500 km)",
                                         "Two-way Clustered")),
      notes= "\\small Two-way clustered standard errors at the Record-Location-Grain and Year level.",
...
```{r}
est1 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10) + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")) , panel.id = c("VarLocationGrain", "Year"), vcov = DK ~ Year)
est2 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10)  + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=200))
est3 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10)  + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=500))
est4 <- feols(logyield ~ Year:Location + l(nino34,0:3):i(teleco_precip_summer_10)  + i(decade)| VarLocationGrain , data = yield_2023 %>% filter(Grain %in% c("Wheat","Rye")), panel.id = c("VarLocationGrain", "Year"), vcov = cluster~ loc_id+Year)
...
# Save plot
ggsave("../figures/plots/figA_irf_yield_WR_AMJJ_P.pdf", device = cairo_pdf, dpi = 300)
```
## IRF NDJF P
...
# Save plot
ggsave("../figures/plots/figA_irf_yield_NDJF_P.pdf", device = cairo_pdf, dpi = 300)
```
...
# Save plot
ggsave("../figures/plots/figA_irf_yield_NDJF_T.pdf", device = cairo_pdf, dpi = 300)
```
...
```{r}
fishprice <- read.csv('../data/fishprice_enso.csv') %>% 
  mutate(Decade = floor(Year / 10) * 10) %>%
...
```{r}
est1 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = DK ~ Year)
est2 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade) + PDSI + temp_summer + temp_winter +precip_summer + precip_winter  | Location , data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = DK ~ Year)
est3 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade) + ongoing_wars + log(1+Deaths) | Location, data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = DK ~ Year)
est4 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade) + PDSI + temp_summer + temp_winter +precip_summer + precip_winter + ongoing_wars + log(1+Deaths) | Location, data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = DK ~ Year)
...
etable(est1, est2, est3,est4,
    ...
    label = "tab:fishprice_FE",
       ...
       dict = dict,
       drop.section = "fixef",                              
    title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Fish Prices",
      extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                   "Location-Species Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                  "Controls" = c("No", "Climate", "Conflict", "Climate+Conflict")),
...
```{r}
est1 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = DK ~ Year)
est2 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov =  vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=200))
est3 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = vcov_conley(lat=~Latitude,lon=~Longitude,cutoff=500))
est4 <- feols(logprice ~ Year:Location + l(nino34,0:3) + i(Decade)| Location , data = fishprice, panel.id = c("LocationSpecies", "Year"), vcov = cluster~ Location+Year)
...
etable(est1, est2, est3,est4,
       ...
       drop.section = "fixef",
        title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Fish Prices under different Standard Errors",
       extralines = list("Decade Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
                   "Location-Species Fixed Effects" = c("Yes", "Yes", "Yes", "Yes"),
...
results <- tibble::tibble(
  Variable = c("Grain Price (all cities)", "Grain Price (all cities)",
...
```{r}
fishcatch <- read.csv('../data/fishcatch_enso.csv') %>% 
  mutate(Decade = floor(Year / 10) * 10) %>%
...
etable(est1,est2,
       ...
       label = "tab:FE_catch",
       drop.section = "fixef",
        title = "Effect of a 1°C Anomaly in the Nino 3.4 Index on Fish Catches under different Standard Errors",
...
ggsave("../figures/plots/figA_irf_catches.pdf", device = cairo_pdf, dpi = 300)
