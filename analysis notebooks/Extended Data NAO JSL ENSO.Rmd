...
```{r theme}
nature_theme <- function(base_size = 8) {
...
```{r panel-a}
COL_ENSO <- "#333333"   # near-black
COL_NAO  <- "#D55E00"   # vermillion
COL_JSL  <- "#0072B2"   # blue
...
make_ts_panel <- function(df, col_index, col_val, label_text,
                          x_title = NULL, y_title = "Standardized index (s.d.)") {
  ggplot(df, aes(Year, value, colour = Index)) +
    ...
    annotate(
      ...
      label = label_text,
      hjust = 0, vjust = 1, size = 2.5, colour = "grey35"
    ) +
...
COL_TOTAL <- "#0072B2"   # blue  – total effect
COL_CTRL  <- "#D55E00"   # red   – controlling for other indices
...
shocks <- tribble(
  ...
  mutate(
    controls_full = glue("{excl}"),
    controls_base = "1"
  )

datasets <- list(
  list(data = fishprice, name = "Fish prices",   panel_id = c("LocationSpecies", "Year"), fe = "LocationSpecies + Decade"),
  list(data = price,     name = "Grain prices",  panel_id = c("Location", "Year"),        fe = "Location + Decade")
)
...
# Region order: sort by mean ENSO effect
region_order <- coef_df %>%
  filter(Index == "ENSO") %>%
  arrange(estimate) %>%
  pull(Region)
...
```{r combined, fig.width=7.2, fig.height=7}
