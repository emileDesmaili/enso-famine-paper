...
suppressPackageStartupMessages(suppressWarnings(library(brms)))
...
suppressPackageStartupMessages(suppressWarnings(library(bayesplot)))
...
suppressPackageStartupMessages(suppressWarnings(library(ggridges)))
...
suppressPackageStartupMessages(suppressWarnings(library(rstan)))
suppressPackageStartupMessages(suppressWarnings(library(panelvar)))
suppressPackageStartupMessages(suppressWarnings(library(mlVAR)))
...
suppressPackageStartupMessages(suppressWarnings(library(depmixS4)))
...
data_grouped <- data %>%
  group_by(Year) %>%
  summarise(
    Famine = sum(Famine, na.rm = TRUE),  # Sum famines across locations
    Famine1Y = sum(Famine1Y, na.rm = TRUE),  # Sum famines across locations
    Famine2Y = sum(Famine2Y, na.rm = TRUE),  # Sum famines across locations
    Famine3Y = sum(Famine3Y, na.rm = TRUE),  # Sum famines across locations
    Famine_start = sum(Famine_start, na.rm = TRUE),  # Sum famines across locations
    nino34 = mean(nino34, na.rm = TRUE),
    nino3 = mean(nino3, na.rm = TRUE),
    nino4 = mean(nino4, na.rm = TRUE),
    nino12 = mean(nino12, na.rm = TRUE),
    ElNino = mean(ElNino, na.rm = TRUE),
    LaNina = mean(LaNina, na.rm = TRUE),
    Deaths = mean(Deaths, na.rm = TRUE),
    Death_ratio = mean(Death_ratio, na.rm=TRUE),
    ongoing_wars = mean(ongoing_wars,na.rm=TRUE),
    NAO_cal = mean(NAO_cal, na.rm=TRUE),
    JSL = mean(JSL, na.rm=TRUE),
    temp_summer = mean(temp_summer, na.rm=TRUE),
    temp_winter = mean(temp_winter, na.rm=TRUE),
    precip_summer = mean(precip_summer, na.rm=TRUE),
    precip_winter = mean(precip_winter, na.rm=TRUE),
    temp_summer_europe = mean(temp_summer_europe, na.rm=TRUE),
    temp_winter_europe = mean(temp_winter_europe, na.rm=TRUE),
    precip_summer_europe = mean(precip_summer_europe, na.rm=TRUE),
    precip_winter_europe = mean(precip_winter_europe, na.rm=TRUE),
...
data_lag = data %>% 
  group_by(Region) %>% 
  mutate(nino34_lag = dplyr::lag(nino34, 1),
         nino3_lag = dplyr::lag(nino3, 1),
         nino4_lag = dplyr::lag(nino4, 1),
         nino12_lag = dplyr::lag(nino12, 1),
         ElNino_lag = dplyr::lag(ElNino, 1),
         LaNina_lag = dplyr::lag(LaNina, 1),
         Deaths_lag = dplyr::lag(Deaths, 1),
         Famine_lag = dplyr::lag(Famine, 1),
         Death_ratio_lag = dplyr::lag(Death_ratio, 1),
         ongoing_wars_lag = dplyr::lag(ongoing_wars, 1),
         PDSI_lag = dplyr::lag(PDSI, 1),
         PDSI_europe_lag = dplyr::lag(PDSI_europe, 1),
         NAO_lag = dplyr::lag(NAO_cal, 1),
         JSL_lag = dplyr::lag(JSL, 1)
         ) %>% 
  ungroup()
...
famine_summary <- famine_periods %>%
  ...
  summarise(
    ...
    max_nino34 = max(nino34, na.rm = TRUE),
    max_PDSI = max(PDSI, na.rm = TRUE),
    max_PDSI_europe = max(PDSI_europe, na.rm = TRUE),
    max_temp_summer = max(temp_summer, na.rm = TRUE),
    max_temp_winter = max(temp_winter, na.rm = TRUE),
    max_precip_summer = max(precip_summer, na.rm = TRUE),
    max_precip_winter = max(precip_winter, na.rm = TRUE),
    max_temp_summer_europe = max(temp_summer_europe, na.rm = TRUE),
    max_temp_winter_europe = max(temp_winter_europe, na.rm = TRUE),
    max_precip_summer_europe = max(precip_summer_europe, na.rm = TRUE),
    max_precip_winter_europe = max(precip_winter_europe, na.rm = TRUE),
    max_ongoing_wars = max(ongoing_wars, na.rm = TRUE),
    max_Deaths = max(Deaths, na.rm = TRUE),
    max_Death_ratio = max(Death_ratio, na.rm = TRUE),
    max_JSL = max(JSL, na.rm = TRUE),
    max_NAO_cal = max(NAO_cal, na.rm = TRUE),
    min_nino34 = min(nino34, na.rm = TRUE),
    avg_2y_nino = mean(nino34[1:2], na.rm = TRUE),
    max_2y_nino = max(nino34[1:2], na.rm = TRUE),
    min_2y_nino = min(nino34[1:2], na.rm = TRUE),
    avg_2y_PDSI = mean(PDSI[1:2], na.rm = TRUE),
    max_2y_PDSI = max(PDSI[1:2], na.rm = TRUE),
    min_2y_PDSI = min(PDSI[1:2], na.rm = TRUE),
    avg_2y_NAO = mean(NAO_cal[1:2], na.rm = TRUE),
    max_2y_NAO = max(NAO_cal[1:2], na.rm = TRUE),
    min_2y_NAO = min(NAO_cal[1:2], na.rm = TRUE),
    avg_2y_JSL = mean(JSL[1:2], na.rm = TRUE),
    max_2y_JSL = max(JSL[1:2], na.rm = TRUE),
    min_2y_JSL = min(JSL[1:2], na.rm = TRUE),
    .groups = "drop"
  )
```
...
ggplot(sf_long, aes(x = time, y = surv, color = ENSO, shape = ENSO)) +
  ...
  geom_point(size = 3) +
  # Arrows for the two highlighted ENSO scenarios
  geom_segment(data = sf_points,
               ...
               linewidth = 1,
               show.legend = FALSE) +
  # Percentage labels, colored by curve
  geom_text(data = sf_points,
            ...
            hjust = 0,
            show.legend = FALSE) +
  scale_color_manual(values = c("Strong El Niño (+2°)" = "firebrick",
                                "La Niña (-1°)" = "cornflowerblue",
                                "Neutral" = "black",
                                "El Niño (+1°)" = "darksalmon")) +
  scale_shape_manual(values = c("Strong El Niño (+2°)" = 18,
                                "La Niña (-1°)" = 19,
                                "Neutral" = 16,
                                "El Niño (+1°)" = 17)) +
  labs(
...
theme_classic(base_size = 15) +
  theme(
    text = element_text(color = "black", size = 16),   # all text black, size 12
    legend.position = "top",
