###############################################################################
# SI analogue of fig3c_robust_and_placebo for the **geographic** partition
# (Central Europe vs Rest of Europe), wheat & rye yields.
#
# Central Europe = Germany, Switzerland, Hungary, Austria (matches the famine-
# onset CE definition in fig 2).
#
# Panel (a): WR teleconnected-style robustness across four specs
#   (Main, + controls, 5 ENSO lags, quadratic record trend) — CE slope at
#   h = 0..3 with 95% DK CI.
# Panel (b): 1000-shuffle NINO3.4 placebo, 95% null envelope vs actual,
#   p-values annotated for h = 0 and h = 3.
#
# Output: analysis/output/figures/investigation/figA_geopartition_robust_and_placebo.pdf
###############################################################################
...
yield_v2 <- read.csv("processed data/yield_ljungqvist_v2.csv") |>
  mutate(decade = floor(Year / 10) * 10,
         Year2  = Year^2,
         CE     = as.integer(Country %in% CE_COUNTRIES))
...
fit_ce <- function(df, h, fe_str = "VarLocationGrain[Year]",
  ...
  cn <- names(coef(m))
  nm <- grep("CE::1:nino34", cn, value = TRUE)
...
cat("\n=== CE coefficient across specs ===\n")
print(all_irf |>
        transmute(spec, h = horizon,
                  est  = sprintf("%+.4f", irf_mean),
                  t    = sprintf("%+.2f", irf_mean / se),
                  ci95 = sprintf("[%+.3f,%+.3f]", irf_down, irf_up)))
...
fit_b_only <- function(df, h) {
  fml <- as.formula(paste0(
    "f(logyield,", h, ") - l(logyield,1) ~ ",
    "i(CE, nino34) + i(decade) + l(nino34, 1:3) | ",
...
for (b in seq_len(N_SHUFFLE)) {
  if (b %% 100 == 0) cat("shuffle ", b, "/", N_SHUFFLE,
                          " (",
...
write.csv(null_mat,
          file.path(OUT_DAT, "geopartition_placebo_null_2000.csv"),
          row.names = FALSE)
write.csv(null_summary,
          file.path(OUT_DAT, "geopartition_placebo_null_summary.csv"),
          row.names = FALSE)
...
actual_df <- data.frame(horizon = HORIZONS, b = actual, p = pvals)
...
pB <- ggplot() +
  ...
  geom_point(data = null_summary, aes(x = horizon, y = null_mean),
             colour = "cornflowerblue", size = 2.4) +
  geom_point(data = actual_df, aes(x = horizon, y = b),
             colour = "firebrick", size = 3.0) +
  geom_text(data = pval_df, aes(x = horizon, y = b, label = label),
  ...
  scale_y_continuous(labels = percent_format(accuracy = 0.1)) +
  labs(x = "Horizon (years)",
       y = "Central-Europe slope per +1 SD NINO3.4",
...
ggsave(file.path(OUT_FIG, "figA_geopartition_robust_and_placebo.pdf"),
       combined, width = 12, height = 5.0, device = cairo_pdf)
cat("\nSaved figA_geopartition_robust_and_placebo.pdf\n")
